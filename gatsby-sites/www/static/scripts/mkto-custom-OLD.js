...
function formSubmissionComplete(pixelEvent, gtmEvent) {
    ...
    // new Image().src = 'https://dc.ads.linkedin.com/collect/?pid=295780&conversionId=868780&fmt=gif'; Commercial LI
    //new Image().src = 'https://dc.ads.linkedin.com/collect/?pid=295780&conversionId=1024388&fmt=gif'; Enterprise LI
    //new Image().src = 'https://px.ads.linkedin.com/collect/?pid=1438908&conversionId=1425108&fmt=gif'; Consumers Advocate submit pixel.

    /*goog_snippet_vars = function() {
    var w = window;
    w.google_conversion_id = 991424858;
    w.google_conversion_label = "AiGVCKDQqX4Q2uLf2AM";
...
//variables scraped by LE
/*var lpLeadSourceCV = leadSourceCookie ? leadSourceCookie : ((name = new RegExp('(?:^|;\\s*)lp-leadSource=([^;]*)').exec(document.cookie)) ? name.split(",")[1] : "");
var gclidCV = (name = new RegExp('(?:^|;\\s*)gclid=([^;]*)').exec(document.cookie)) ? name.split(",")[1] : "";
var msclkidCV = (name = new RegExp('(?:^|;\\s*)msclkid=([^;]*)').exec(document.cookie)) ? name.split(",")[1] : "";
var lsRefCV = (name = new RegExp('(?:^|;\\s*)lp-lsRef=([^;]*)').exec(document.cookie)) ? name.split(",")[1] : "";*/
...
function showMenuHack(engagementId) {
	...
	if ($('body:not(.cio) #lpChat div[data-lp-point="lines_area"]').length < 1) {
		...
		// if ((!mutedTimeout && document.querySelector('body:not(.cio) .lp_header .lp_top-text') && document.querySelector('.lp_header .lp_top-text').textContent.toLowerCase() === 'message us')){
		...
		if (document.querySelector('.lp_header .lp_top-text') && document.querySelector('.lp_header .lp_top-text').textContent.toLowerCase() === 'message us'){
			if(!conversationStarter && $('body.cio #lp_line_0 .lp_title_text').text() === 'How can I help you today?'){
				...
				// $('.lp_chat_line_wrapper .lp_title_text').each(function(){
				// 	if( $(this).text() === 'How can I help you today?' ){
				// 		console.log('this one');
				// 		$(this).parents('.lp_chat_line_wrapper').remove();
				// 		showInitMenu();
				// 	}
				// })
...
(function ($) {
		...
		form.onValidate(function () {
			...
			if (emailVal !== undefined) {
				...
				if (!isEmailGood(emailVal)) {
					...
					$('input[name=console]').val($('input[name=console]').val() + '*Email is valid* ');
					//console.log('valid');
					continueDemandbase(formID);
					$('input[name=console]').val($('input[name=console]').val() + '*Email -> company done* ');
					/*gclId = (name = new RegExp('(?:^|;\\s*)gclid=([^;]*)').exec(
						document.cookie)) ? name.split(",")[1] : "";
					msclkId = (name = new RegExp('(?:^|;\\s*)msclkid=([^;]*)').exec(
						document.cookie)) ? name.split(",")[1] : "";
					leadSource = (name = new RegExp(
						'(?:^|;\\s*)lp-leadSource=([^;]*)').exec(
						document.cookie)) ? name.split(",")[1] : "";
					leadRef = (name = new RegExp(
							'(?:^|;\\s*)lp-lsRef=([^;]*)').exec(document.cookie)) ?
						name.split(",")[1] : "";*/
					form.vals({
						'GCLID__c': gclid,
						'MSCLIKID__c': msclkid,
	...
	function isEmailGood(email) {
		$('input[name=console]').val($('input[name=console]').val() + '*Start blacklist checker* ');
	...
	function continueDemandbase(formID) {
		$('input[name=console]').val($('input[name=console]').val() + '*Start email -> company split* ');
