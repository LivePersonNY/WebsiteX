...
const BlogPost = ({ data: { previous, next, post, staged } }) => {
  ...
  let canonical =
    post.seo.canonical == post.link ? canRoot + post.link : post.seo.canonical;
  ...
  let meta = [
    ...
    },
    {
      property: `og:type`,
      content:
        post.seo.opengraphType || post.seo.schema.articleType || `website`,
    },
    {
      property: `og:url`,
      content: canonical,
  ...
  breadCrumbs = breadCrumbs.map((item, index) => {
    if (index === 0) {
      return (
        <>
          <a
            className="breadcrumb-link link link-no-arrow"
