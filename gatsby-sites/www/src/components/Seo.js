...
const Seo = ({ description, lang, meta, title, canonical, robots }) => {
  ...
  let socialTags = meta.map(function(item, index) {
    return (
      <meta name={item.name || item.property.substring(3)} property={item.property} content={item.content} key={index} />
    );
  });
  ...
  return (
    ...
    >
      ...
      <link
        href="https://fonts.googleapis.com/css2?family=Inter:wght@400;700&family=Space+Grotesk:wght@400;600;700&family=Kalam:wght@300;400;700&display=swap"
        rel="stylesheet"
      />
