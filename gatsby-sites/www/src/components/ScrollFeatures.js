...
export default function ScrollFeatures({
    ...
    useEffect(() => {
        ...
        const onWinScroll = (e) => {
            ...
            const lastStickyBox = document.querySelector(
                '.sticky-boxes-wrapper .sticky-box:last-child'
            );
            const stickyBoxBounds = stickyBox.getBoundingClientRect();
            const winHeight = window.innerHeight;
            if (stickyBoxBounds.top * -1 > stickyBoxBounds.height - winHeight) {
                const translateOffset =
                    stickyBoxBounds.top * -1 - stickyBoxBounds.height; //  - winHeight
        ...
        <>
            <div
                className={`${isReady ? 'ready' : ''} ${
                    backgroundColor ? backgroundColor : 'bg-neutral-96'
