        ...
        graphql`
            query topLevelQuery {
                ...
                topItems: allWpMenuItem(
                    filter: {
                        menu: {
                            node: { locations: { in: [GATSBY_HEADER_MENU] } }
                        }
                        parentId: { eq: null }
                    }
                ) {
        ...
        <nav className="navbar navbar-expand-lg navbar-light">
            <div className="container" style={{ position: 'relative' }}>
                <a href="/">
                    <img
                        className="site-logo"
                        src={settings.allSettings.siteLogo}
                        alt="LivePerson Inc."
                    />
                </a>
                <button
                ...
                >
                    <ul className="navbar-nav">
                        {topItems.nodes.map((item, index) => {
                            ...
                            let featuredResource =
                                item.cssClasses.indexOf('featured-resource') >=
                                0;
                            let featuredNews =
                                item.cssClasses.indexOf('featured-news') >= 0;
                            let featuredProducts =
                                item.cssClasses.indexOf('featured-products') >=
                                0;
