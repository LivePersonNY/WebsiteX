    ...
    <div id="mo_saml_settings" >
        ...
        <div class="wrap">
            <h1>
                ...
                <a id="license_upgrade" class="add-new-h2 add-new-hover" style="background-color: orange !important; border-color: orange; font-size: 16px; color: #000;" href="<?php echo mo_saml_add_query_arg( array( 'tab' => 'licensing' ), htmlentities( $_SERVER['REQUEST_URI'] ) ); ?>"><?php _e('Premium Plans | Upgrade Now','miniorange-saml-20-single-sign-on'); ?></a>
                ...
                <a class="add-new-h2" href="<?php echo admin_url().'admin.php?page=mo_saml_enable_debug_logs'?>"><?php _e('Troubleshoot','miniorange-saml-20-single-sign-on');?></a>
...
<div id="getting-started" class="modal">
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        ...
        <div class="modal-content" style="height:fit-content;">
            <span style="float: right;cursor: pointer;padding-top: 26px" onclick="skip_plugin_tour();" ><i class="dashicons dashicons-dismiss" ></i></span>
...
">
                ...
                $index=0;
                    foreach (mo_saml_options_plugin_idp::$IDP_GUIDES as $key=>$value){
                        ...
                        if($index%5===0){?>
                            ...
                            <?php } ?>
                         <button class="guide-btn" onclick="window.open('<?php echo $url_string?>','_blank')"><img class="idp-guides-logo <?php echo $key?>" src="<?php echo plugin_dir_url( __FILE__ ) . 'images'.DIRECTORY_SEPARATOR.'idp-guides-logos'.DIRECTORY_SEPARATOR.$value[0].'.png'; ?>" /><?php echo $key?></button>
                        <?php
            ...
            <div class="modal-footer" style="position: sticky;">
                ...
                <button type="button" style="width:175px;" class="button button-primary button-large modal-button" id="skip-plugin-tour" onclick="skip_plugin_tour()" ><?php _e('Skip tour','miniorange-saml-20-single-sign-on');?></button>
                <p style="font-size: large;text-align: center;font-weight: 400;"><?php _e('Take a quick tour of setting up the plugin with ADFS','miniorange-saml-20-single-sign-on');?> <i style="font-size: small">(<?php _e('Press Esc to skip','miniorange-saml-20-single-sign-on');?>)</i></p>
            ...
            <div style="margin:2% 0 2% 17px;">
                ...
                <div id="mo_saml_add_widget_steps">
                    <ol>
	                    ...
	                    global $wp_version;
	                    if($wp_version < '5.8') { ?>
                            <li><?php echo sprintf(__('Select "Login with %s','miniorange-saml-20-single-sign-on'),get_option( 'saml_identity_name' ));?>". <?php _e('Drag and drop to your favourite location and save.','miniorange-saml-20-single-sign-on');?></li>
                        <?php }
	                    else { ?>
                            <li><?php echo sprintf(__('Click on Add Block ("+" sign) at the top left corner, besides the heading Widgets.','miniorange-saml-20-single-sign-on'));?></li>
                            <li><?php echo sprintf(__('In the search box, search for "Login with %s", and drag and drop this block to your favourite location.','miniorange-saml-20-single-sign-on'),get_option( 'saml_identity_name' ));?></li>
                            <li><?php echo sprintf(__('Click on the "Update" button at the top right to save the widget settings.','miniorange-saml-20-single-sign-on'));?></li>
	                    <?php  } ?>
                    </ol>
                </div>
            </div>
                            ...
                            <ul>
                                ...
                                foreach(mo_saml_options_plugin_idp::$IDP_GUIDES as $key=>$value){
                                    ...
                                    <a target="_blank" style="cursor: pointer" data-idp-value="<?php echo $idp_video_index?>"
                                           data-href="https://plugins.miniorange.com/<?php echo $value[1]; ?>"
                                        data-video="https://www.youtube.com/watch?v=<?php echo $idp_video_index?>">
                                            <img src = "<?php echo plugins_url( $image_path.$value[0].'.png', __FILE__ );?>">
...
function miniorange_support_saml($active_tab, $display_support_layout=true, $display_keep_configuration_intact=true) {
	...
	if($display_keep_configuration_intact) {
	    ...
	    MoSAMLLogger::mo_saml_log_page();
    }
    else{
        $addon_slug='wp-page-restriction';
        if($active_tab=='account-setup' || $active_tab=='opt'){
	        $addon_slug='wp-scim';
        }
        elseif($active_tab=='support'){
            $addon_slug2 = 'wp-scim';
	        mo_saml_display_add_ons_iframe(mo_saml_options_addons::$WP_ADDON_URL[$addon_slug2]);?><br><?php
        }
