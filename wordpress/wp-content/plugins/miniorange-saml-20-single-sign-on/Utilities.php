...
class Utilities {
	...
	public static function processResponse( $currentURL, $certFingerprint, $signatureData, SAML2_Response $response, $certNumber, $relayState ) {
		...
		if ($notBefore !== NULL && $notBefore > time() + 60) {
			$Logger->add_log('Received an assertion that is valid in the future. Check clock synchronization on IdP and SP.',$Logger::ERROR);
		...
		if ($notOnOrAfter !== NULL && $notOnOrAfter <= time() - 60) {
			$Logger->add_log('Received an assertion that has expired. Check clock synchronization on IdP and SP.',$Logger::ERROR);
		...
		if ($msgDestination !== NULL && $msgDestination !== $currentURL) {
			$Logger->add_log('Destination in response doesn\'t match the current URL. Destination is "' . htmlspecialchars( $msgDestination ) . '", current URL is "' . htmlspecialchars( $currentURL ) . '".',$Logger::ERROR);
        ...
        if(strcmp($issuerToValidateAgainst, $issuer) === 0) {
            if(!empty($audiences)) {
                if(in_array($spEntityId, $audiences, TRUE)) {
                    ...
                    $saml_logger->add_log(mo_saml_error_log::showMessage('UTLITITES_INVAID_AUDIENCE_URI',array('spEntityId'=>$spEntityId,'audiences'=>$audiences)), MoSAMLLogger::ERROR);
            ...
            $saml_logger->add_log(mo_saml_error_log::showMessage('UTLITITES_INVAID_ISSUER', array('issuerToValidateAgainst'=>$issuerToValidateAgainst,'issuer'=>$issuer)), MoSAMLLogger::ERROR);
            if($relayState=='testValidate'){
