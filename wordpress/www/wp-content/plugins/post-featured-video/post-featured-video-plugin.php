...
if (!function_exists('pfvideo_pst_admin_notice__success')){
	function pfvideo_pst_admin_notice__success() {
	    ...
	    <div class="notice notice-success is-dismissible">
	        <p><?php _e( 'To view setting please ', 'post-featured-video' ); ?><a href="<?php echo admin_url('admin.php?page=featuredvideo'); ?>"><?php _e( 'click here', 'post-featured-video' ); ?></a></p>
	    </div>
	    <?php 
...
if (!function_exists('pfvideo_featured_vid_save')){
	function pfvideo_featured_vid_save($post_id){
	    ...
	    $nonce_name   = isset( $_POST['pfv_custom_nonce'] ) ? $_POST['pfv_custom_nonce'] : '';
		...
		if(isset($_POST['pfv_custom_vid_url'])){	
			$custom_vid_url = esc_url($_POST['pfv_custom_vid_url']);	
			update_post_meta($post_id, '_pfv_custom_vid_url', $custom_vid_url );		
		}

		/*Vimeo Video*/
		if(isset($_POST['pfv_vimeo_video_url'])){	
			$custom_vid_url = esc_url($_POST['pfv_vimeo_video_url']);
				...
				if(!empty(get_post_meta($post_id, '_pfv_vimeo_video_url', true))){
					...
					if(!empty($popupvideo) && $popupvideo == 1){
						...
						<div id="pfv_vvideo_lightbox_'.$post_id.'" class="pfv_vvideo_lightbox">
						  ...
						  <div style="padding:56.25% 0 0 0;position:relative;"><iframe id="pfviframeVideo_'.$post_id.'" src="https://player.vimeo.com/video/'.$vimimgid.'?autoplay='.$autplyvideo.'" style="position:absolute;top:0;left:0;width:100%;height:100%;" frameborder="0" allow="autoplay; fullscreen" allowfullscreen></iframe></div><script src="https://player.vimeo.com/api/player.js"></script>					  
						</div>

						<div id="pfv_video_fadelayout_'.$post_id.'" class="pfv_video_fadelayout" onClick="vmio_lightbox_close('.$post_id.');"></div></div>';
					}
					else{
						return '<div style="padding:56.25% 0 0 0;position:relative;"><iframe id="pfviframeVideo_'.$post_id.'" src="https://player.vimeo.com/video/'.$vimimgid.'" style="position:absolute;top:0;left:0;width:100%;height:100%;" frameborder="0" allow="autoplay; fullscreen" allowfullscreen></iframe></div><script src="https://player.vimeo.com/api/player.js"></script>';
					}
				}
				else{
					return $image;
			...
			if(null !== get_option('vpfy_disply_video') && get_option('vpfy_disply_video') == 'pfvuploder'){
				if( $media = wp_get_attachment_url($meta_ket)) { 
					...
					if(!empty($popupvideo) && $popupvideo == 1){
						...
						return __('<div id="pfv_vidBox" class="pfv_vidBox" style="clear:both"><video  class="pfv_htmlvideo" id="pfv_htmlvideo_'.$post_id.'" controls src="'.$media.'" style="width:100%;display:block;"></video></div>'); 
					}				
				}
				else{
					return $image;
